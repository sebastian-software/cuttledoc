...
setBackend('parakeet') // Use Parakeet (fastest)
console.log(getBackend()) // "parakeet"
```
...
Auto-select the best available backend for the current platform and a given language. macOS uses the local CoreML
backends. Linux and Windows use OpenAI when an API key is configured and otherwise return an actionable error.

```typescript
import { selectBestBackend } from 'cuttledoc'

const best = selectBestBackend('de', process.env.OPENAI_API_KEY)
```
...
await downloadModel('parakeet') // Downloads Parakeet CoreML models
await downloadModel('whisper') // Downloads Whisper CoreML models
```
...
```typescript
import { downloadModel } from '@cuttledoc/llm'
...
```typescript
import { isModelDownloaded } from '@cuttledoc/llm'
...
```typescript
type BackendType = 'auto' | 'parakeet' | 'whisper' | 'openai'
```
...
25 supported languages for Parakeet.
...
99 supported languages for Whisper (subset shown).
