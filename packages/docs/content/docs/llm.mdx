...
| -------------- | ---------- | ------ | ----- | --------------------------- |
| `phi4:14b`     | ⭐⭐⭐⭐⭐ | 36 t/s | 9 GB  | Best overall (default)      |
...
- macOS: `~/Library/Caches/cuttledoc/models/llm`
- Windows: `%LOCALAPPDATA%\cuttledoc\models\llm`

Set `CUTTLEDOC_LLM_MODELS_DIR` to an absolute path to use a custom location. The old
